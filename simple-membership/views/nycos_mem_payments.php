...
<main class="site-main" id="main" role="main">
    ...
    <h1 class="page-header">Your Membership Payments</h1>
    <p class="hidden">
        <a href="javascript:window.print()" title="Print this page">Print</a> this page.
    </p>
    <p id="ctl00_cp1_donations"> This page shows the membership payments we have received from you in the last 12 months. </p>
    <h4>Your payments since <?=date('F j Y', strtotime(' - 1 years')); ?></h4>
    <fieldset>
        <table class="table">
            <tbody>
                <tr>
                ...
                if ($payment->incomeType=="MEMBERSHIP"){
                    if ($payment->date > $yearAgoToday) {?> 

                <tr>
                    <td class="tdWide">
                        <h4> <?= date("F j Y", strtotime($payment->date)) ?> </h4> <?= $payment->paymentMethod ?> Payment for <?= $payment->externalRef ?>
                    </td>
